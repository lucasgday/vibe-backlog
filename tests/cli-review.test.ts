...
describe.sequential("cli review", () => {
  ...
  it("reports pass totals and resolved findings after autofix clears findings", async () => {
    process.env.VIBE_REVIEW_AGENT_CMD = "cat";
    await writeTurnContext({
      issue_id: 34,
      branch: "codex/issue-34-vibe-review",
    ...
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      ...
      if (cmd === "zsh") {
        agentRuns += 1;
        if (agentRuns === 1) {
          return { stdout: buildAgentOutput({ runId: "run-resolve-1", findingsCount: 1, autofixApplied: true }) };
        }
        return { stdout: buildAgentOutput({ runId: "run-resolve-2", findingsCount: 0, autofixApplied: true }) };
      }
      return { stdout: "" };
    });
    vi.spyOn(console, "error").mockImplementation(() => undefined);
    ...
    expect(logs.some((line) => line.includes("- security: total=1 unresolved=0 resolved=1"))).toBe(true);
    expect(logs.some((line) => line.includes("### Resolved Findings"))).toBe(true);
    expect(logs.some((line) => line.includes("Validate user input"))).toBe(true);
  });

  it("uses lifecycle totals when PR review threads already contain resolved findings", async () => {
    process.env.VIBE_REVIEW_AGENT_CMD = "cat";
    await writeTurnContext({
      issue_id: 34,
      branch: "codex/issue-34-vibe-review",
    ...
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      ...
      if (cmd === "gh" && args[0] === "pr" && args[1] === "list")
        return { stdout: JSON.stringify([{ number: 99, url: "https://example.test/pull/99", headRefOid: "abc123", body: "Fixes #34" }]) };
      if (cmd === "zsh") return { stdout: buildAgentOutput({ runId: "run-lifecycle-totals", findingsCount: 0, autofixApplied: true }) };
  ...
  it("unions lifecycle and current-run finding sets when both are present", async () => {
    ...
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      ...
      if (cmd === "gh" && args[0] === "pr" && args[1] === "list")
        return { stdout: JSON.stringify([{ number: 99, url: "https://example.test/pull/99", headRefOid: "abc123", body: "Fixes #34" }]) };
      if (cmd === "zsh") return { stdout: buildAgentOutput({ runId: "run-lifecycle-union", findingsCount: 1, autofixApplied: false }) };
      if (cmd === "gh" && args[0] === "api" && args[1] === "repos/acme/demo/issues/99/comments?per_page=100&page=1") {
        return { stdout: "[]" };
      }
      if (cmd === "gh" && args[0] === "api" && args[1] === "--method" && args[2] === "POST" && args[3] === "repos/acme/demo/issues/99/comments") {
                      ...
                      nodes: [
                        {
                          ...
                          comments: {
                            nodes: [
                              {
                                id: "comment-old",
                                body: "**[P2] Legacy finding**\n\nPass: `security`\n\n<!-- vibe:fingerprint:abc123def4567890 -->",
                                url: "https://example.test/comment/legacy",
  ...
  it("does not collapse distinct current findings when canonical lifecycle key is ambiguous", async () => {
    ...
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      ...
      if (cmd === "zsh") {
        return {
          stdout: buildAgentOutput({
            runId: "run-lifecycle-ambiguous-canonical",
            findingsCount: 2,
            autofixApplied: false,
            findings: [
              {
                ...
                severity: "P2",
                title: "Validate user input",
                body: "Input path needs validation for shell escapes.",
                file: "src/cli-program.ts",
                line: 42,
              },
              {
                ...
                severity: "P2",
                title: "Validate user input",
                body: "Input path needs validation for traversal payloads.",
                file: "src/cli-program.ts",
                line: 42,
              },
            ],
                ...
                repository: {
                  pullRequest: {
                    reviewThreads: {
                      ...
                      nodes: [
                        {
                          id: "PRRT_connector_ambiguous",
                          isResolved: false,
                          isOutdated: false,
                          comments: {
                            nodes: [
                              {
                                id: "comment-connector-ambiguous",
                                body: "**[P2] Validate user input**\n\nPass: `security`",
                                url: "https://example.test/comment/connector-ambiguous",
                                path: "src/cli-program.ts",
                                line: 42,
                                originalLine: 42,
                                author: { login: "chatgpt-codex-connector" },
  ...
  it("auto-closes open follow-up issues when unresolved findings reach zero", async () => {
    ...
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      ...
      if (cmd === "gh" && args[0] === "pr" && args[1] === "list")
        return { stdout: JSON.stringify([{ number: 99, url: "https://example.test/pull/99", headRefOid: "abc123", body: "Fixes #34" }]) };
      if (cmd === "zsh") return { stdout: buildAgentOutput({ runId: "run-close-followup", findingsCount: 0 }) };
      if (cmd === "gh" && args[0] === "api" && args[1] === "repos/acme/demo/issues?state=open&per_page=100&page=1") {
        return {
          stdout: JSON.stringify([
            {
              ...
              html_url: "https://example.test/issues/801",
            },
          ]),
        };
      }
      if (cmd === "gh" && args[0] === "issue" && args[1] === "close" && args[2] === "801") return { stdout: "" };
      return { stdout: "" };
    });
    vi.spyOn(console, "error").mockImplementation(() => undefined);
    vi.spyOn(console, "log").mockImplementation((...args: unknown[]) => {
    ...
    expect(
      execaMock.mock.calls.some(
        ([cmd, args]) => cmd === "gh" && Array.isArray(args) && args[0] === "issue" && args[1] === "close" && args[2] === "801",
      ),
    ).toBe(true);
    expect(logs.some((line) => line.includes("### Follow-up Closure"))).toBe(true);
    expect(logs.some((line) => line.includes("- Closed follow-ups: #801"))).toBe(true);
    expect(
      execaMock.mock.calls.some(
        ([cmd, args]) => cmd === "gh" && Array.isArray(args) && args[0] === "issue" && args[1] === "create",
      ),
  ...
  it("continues review and reports warning when follow-up auto-close fails", async () => {
    process.env.VIBE_REVIEW_AGENT_CMD = "cat";
    await writeTurnContext({
      issue_id: 34,
      branch: "codex/issue-34-vibe-review",
    ...
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      ...
      if (cmd === "gh" && args[0] === "pr" && args[1] === "list")
        return { stdout: JSON.stringify([{ number: 99, url: "https://example.test/pull/99", headRefOid: "abc123", body: "Fixes #34" }]) };
      if (cmd === "zsh") return { stdout: buildAgentOutput({ runId: "run-close-warning", findingsCount: 0 }) };
      if (cmd === "gh" && args[0] === "api" && args[1] === "repos/acme/demo/issues?state=open&per_page=100&page=1") {
        return {
          stdout: JSON.stringify([
            {
              number: 802,
              body: "<!-- vibe:review-followup:source-issue:34 -->\nopen follow-up",
              html_url: "https://example.test/issues/802",
            },
          ]),
        };
      }
      if (cmd === "gh" && args[0] === "issue" && args[1] === "close" && args[2] === "802") {
        closeAttempts += 1;
    ...
    expect(logs.some((line) => line.includes("### Follow-up Closure Warnings"))).toBe(true);
    expect(logs.some((line) => line.includes("follow-up auto-close failed for #802"))).toBe(true);
  });
