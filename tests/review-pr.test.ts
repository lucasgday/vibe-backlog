...
describe("review PR helpers", () => {
  ...
  it("creates follow-up issue with only labels available in repo", async () => {
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      ...
      if (cmd === "gh" && args[0] === "issue" && args[1] === "create") {
        ...
        expect(bodyFromFile).toContain("Auto-generated by `vibe review`");
        expect(bodyFromFile).toContain("## Unresolved Findings");
  ...
  it("normalizes escaped and literal newline markers in follow-up issue body", async () => {
    const execaMock = vi.fn(async (cmd: string, args: string[]) => {
      if (cmd === "gh" && args[0] === "repo" && args[1] === "view") {
        return { stdout: "acme/demo\n" };
      }
