...
function normalizeFollowUpSummaryNewlines(summary: string): string {
  return summary
    .replace(/\r\n/g, "\n")
    .replace(/\\n(?=\s*(?:[-*#]|\d+\.))/g, "\n")
    .replace(/(?:^|[ \t]+)\/n(?=\s*(?:[-*#]|\d+\.))/gm, "\n");
...
function buildFollowUpIssueBody(sourceIssueId: number, findings: ReviewFinding[], reviewSummary: string): string {
  const normalizedReviewSummary = normalizeFollowUpSummaryNewlines(reviewSummary);
  const summaryWasNormalized = normalizedReviewSummary !== reviewSummary;
  const lines = [
  ...
  if (summaryWasNormalized) {
    lines.push("<!-- vibe:followup-summary-normalized:newlines -->");
  }
  lines.push(normalizedReviewSummary, "", "## Unresolved Findings");
...
async function createIssueWithLabels(params: {
  ...
  return await createIssueWithBodyFile(params);
}
